...
# created by Ion Motion Control (probably)
# editted by Matthew Wilson (@matwilso)

class Cmd():
    """
    Enums for sending serial commands to roboclaw
    These are defined in the roboclaw user manual.
    """
    M1FORWARD = 0
    M1BACKWARD = 1
    SETMINMB = 2
...
class Roboclaw:
    ...
    This class was designed to be used for a single serial port connection
    to 1 or more Roboclaw devices being driven in Packet Serial Mode. It is a 
    low-level serial driver, meant to be included in a ROS node for commanding
    motors.
    ...
    addr = 128 # Packet Serial Mode address of a Roboclaw device
    roboclaw.ForwardM1(addr, 63) # set to half-speed
    time.sleep(1.0)
    roboclaw.ForwardM1(addr, 0) # turn-off
    ```

    Attributes:
        port (string): serial device (e.g. '/dev/ttyACM0')
        baudrate (int): serial baudrate (e.g. 115200)
        timeout (float): how long to wait to read from serial (lower means more dropped readings, but won't stall)
        retries (int): number of times to retry command 
    ...
    def __init__(self, port, baudrate, timeout=0.05, retries=1):
        """Initialize Roboclaw controller params: port, baudrate, serial timeout, 
        ...
        self._crc = 0
        # Open the port
        self._port = serial.Serial(port=self.port, baudrate=self.baudrate, timeout=self.timeout, interCharTimeout=self.timeout)
    ...
    def SetM1VelocityPID(self, address, p, i, d, qpps):
        return self._write4444(address, Cmd.SETM1PID, long(d*65536), long(p*65536), long(i*65536), qpps)

    def SetM2VelocityPID(self, address, p, i, d, qpps):
        return self._write4444(address, Cmd.SETM2PID, long(d*65536), long(p*65536), long(i*65536), qpps)
    ...
    def DutyM1(self, address, val):
        raise NotImplementedError("This is not supported.  The library we forked from was calling some non-existent fucntion")
        #return self._simplFunctionS2(address, Cmd.M1DUTY, val)

    def DutyM2(self, address, val):
        raise NotImplementedError("This is not supported.  The library we forked from was calling some non-existent fucntion")
        #return self._simplFunctionS2(address, Cmd.M2DUTY, val)
    ...
    def DutyAccelM1M2(self, address, accel1, duty1, accel2, duty2):
        return self._writeS24S24(Cmd.MIXEDDUTYACCEL, address, duty1, accel1, duty2, accel2)
    ...
    def ReadError(self, address):
        return self._read2(address, Cmd.GETERROR)
